...
The MANRS IXP Programme defines five actions. Actions 1 and 2 are mandatory; at least one of Actions 3–5 is also required (minimum three of five). PACIXP implements all five:
...
- All four route server filter layers (RPKI ROV, IRR prefix list, bogon rejection, max-prefix limit) are applied on all sessions, implementing MANRS IXP Action 1.
